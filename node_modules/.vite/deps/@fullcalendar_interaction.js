	...
	constructor(dragging, droppableStore) {
		...
		this.handlePointerDown = (ev) => {
			...
			if (this.initialHit || !this.requireInitial) {
				dragging$1.setIgnoreMove(false);
				this.emitter.trigger("pointerdown", ev);
			} else dragging$1.setIgnoreMove(true);
		};
...
var DateSelecting = class extends Interaction {
	constructor(settings) {
		...
		this.handlePointerDown = (ev) => {
			let { component: component$1, dragging: dragging$1 } = this;
			let { options: options$1 } = component$1.context;
			let canSelect = options$1.selectable && component$1.isValidDateDownEl(ev.origEvent.target);
			dragging$1.setIgnoreMove(!canSelect);
			dragging$1.delay = ev.isTouch ? getComponentTouchDelay$1(component$1) : null;
		};
		this.handleDragStart = (ev) => {
...
var EventDragging = class EventDragging extends Interaction {
	constructor(settings) {
		...
		this.handlePointerDown = (ev) => {
			...
			let { component: component$1, dragging: dragging$1 } = this;
			let { mirror } = dragging$1;
			let { options: options$1 } = component$1.context;
			let initialContext = component$1.context;
			...
			dragging$1.minDistance = ev.isTouch ? 0 : options$1.eventDragMinDistance;
			dragging$1.delay = ev.isTouch && eventInstanceId !== component$1.props.eventSelection ? getComponentTouchDelay(component$1) : null;
			if (options$1.fixedMirrorParent) mirror.parentNode = options$1.fixedMirrorParent;
			else mirror.parentNode = elementClosest(origTarget, ".fc");
			mirror.revertDuration = options$1.dragRevertDuration;
			let isValid = component$1.isValidSegDownEl(origTarget) && !elementClosest(origTarget, ".fc-event-resizer");
			dragging$1.setIgnoreMove(!isValid);
...
function computeEventMutation(hit0, hit1, massagers) {
	...
	if (dateSpan0.allDay !== dateSpan1.allDay) {
		...
		if (dateSpan1.allDay) date0 = startOfDay(date0);
	}
...
var EventResizing = class extends Interaction {
	constructor(settings) {
		...
		this.mutatedRelevantEvents = null;
		this.handlePointerDown = (ev) => {
			let { component: component$1 } = this;
			let eventRange = this.eventRange = getElSeg(this.querySegEl(ev)).eventRange;
			this.dragging.minDistance = component$1.context.options.eventDragMinDistance;
...
var UnselectAuto = class {
	constructor(context) {
		...
		this.onDocumentPointerUp = (pev) => {
			let { context: context$1 } = this;
			let { documentPointer: documentPointer$1 } = this;
			let calendarState = context$1.getCurrentData();
			if (!documentPointer$1.wasTouchScroll) {
				if (calendarState.dateSelection && !this.isRecentPointerDateSelect) {
					let unselectAuto = context$1.options.unselectAuto;
					if (unselectAuto && (!unselectAuto || !this.matchesCancel)) context$1.calendarApi.unselect(pev);
				}
				if (calendarState.eventSelection && !this.matchesEvent) context$1.dispatch({ type: "UNSELECT_EVENT" });
			}
			this.isRecentPointerDateSelect = false;
		};
...
var ExternalElementDragging = class {
	constructor(dragging, suppliedDragMeta) {
		...
		this.handleHitUpdate = (hit, isFinal, ev) => {
			let { dragging: dragging$1 } = this.hitDragging;
			let receivingContext = null;
			...
			this.displayDrag(receivingContext, interaction);
			dragging$1.setMirrorIsVisible(isFinal || !droppableEvent || !document.querySelector(".fc-event-mirror"));
			if (!isInvalid) enableCursor();
			else disableCursor();
			if (!isFinal) {
				dragging$1.setMirrorNeedsRevert(!droppableEvent);
				this.receivingContext = receivingContext;
...
var ExternalDraggable = class {
	constructor(el, settings = {}) {
		this.handlePointerDown = (ev) => {
			...
			dragging$1.minDistance = minDistance != null ? minDistance : ev.isTouch ? 0 : BASE_OPTION_DEFAULTS.eventDragMinDistance;
			dragging$1.delay = ev.isTouch ? longPressDelay != null ? longPressDelay : BASE_OPTION_DEFAULTS.longPressDelay : 0;
		};
		this.handleDragStart = (ev) => {
